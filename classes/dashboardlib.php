...
class dashboardlib {
    ...
    public static function get_assignment_data($course, $cm) { 
        ...
        $average = $DB->get_field_sql(
            'SELECT AVG(finalgrade) FROM {grade_grades} WHERE itemid = :itemid AND finalgrade IS NOT NULL',
            ['itemid' => $a['gradeitem']->id]
        );
    ...
    public static function get_student_table_data($course, $cm, $search = '') {
        ...
        foreach ($students as $student) {
            ...
            if ($search !== ''
                ...
                $button = ($submission && $grade && $grade->finalgrade !== null)
                    ? \html_writer::link(
                        $detailurl,
                ...
                $rows[] = [
                    ...
                    'status' => !$submission ? 'No submission'
                                : ($grade && $grade->finalgrade !== null ? 'Graded' : 'Submitted'),
                    'grade' => $grade && $grade->finalgrade !== null ? number_format($grade->finalgrade, 2) : 'Not graded',
                    'button' => $button
                ];
        } 
    }
